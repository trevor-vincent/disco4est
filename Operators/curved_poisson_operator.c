...
)
{
  ...
  for (i = 0; i < (P4EST_DIM); i++){
    ...
    /* d4est_operators_interp_lobatto_to_GL(d4est_ops, &elem_data->dudr_elem[i][0], elem_data->deg_quad, elem_data->deg_quad, du_di_gauss, (P4EST_DIM)); */
    ...
    d4est_operators_interp_lobatto_to_GL(d4est_ops, du_di_prolonged, elem_data->deg_quad, elem_data->deg_quad, du_di_gauss, (P4EST_DIM));
  ...
  /* DEBUG_PRINT_ARR_DBL(tmp_ptr, d4est_lgl_get_nodes((P4EST_DIM),elem_data->deg_quad)); */
...
)
{
  ...
  /* int deg = element_data->deg; */
  int faces = 2*dim;
  /* int face_nodes_gauss = d4est_lgl_get_nodes(dim-1,element_data->deg_quad); */
  int face_nodes_lobatto = d4est_lgl_get_nodes(dim-1,element_data->deg);
  /* int volume_nodes_gauss = d4est_lgl_get_nodes(dim,element_data->deg_quad); */
...
/* #endif */
  ...
  /* int volume_nodes_quad = d4est_lgl_get_nodes((P4EST_DIM), element_data->deg_quad); */
...
/*   double* dq = P4EST_ALLOC(double, d4est_lgl_get_nodes(dim,deg)); */
/*   double* dq_gauss = P4EST_ALLOC(double, d4est_lgl_get_nodes(dim,deg)); */
/*   double* vol_tmp = P4EST_ALLOC_ZERO(double, d4est_lgl_get_nodes(dim,deg)); */
...
/*       d4est_operators_interp_lobatto_to_GL(d4est_ops, dq, deg, deg, dq_gauss, (P4EST_DIM)); */
...
void curved_gauss_poisson_compute_Au_elem(p4est_iter_volume_info_t* info, void* user_data)
{
  ...
  for (i = 0; i < (P4EST_DIM); i++){
    for (j = 0; j < (P4EST_DIM); j++){
      ...
      d4est_operators_interp_lobatto_to_GL(d4est_ops, dq, element_data->deg, element_data->deg_quad, dq_gauss, (P4EST_DIM));
      for (k = 0; k < volume_nodes_gauss; k++){
