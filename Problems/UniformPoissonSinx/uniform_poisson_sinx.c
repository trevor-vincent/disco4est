...
)
{
  ...
  for (int level = 0; level < d4est_amr->num_of_amr_steps + 1; ++level){
    ...
    double local_eta2 = d4est_estimator_bi_compute
      (
       p4est,
       &prob_vecs,
       &prob_fcns,
    ...
    d4est_linalg_vec_axpyeqz(-1., u_analytic, prob_vecs.u, error, local_nodes);
    d4est_linalg_vec_fabs(error, local_nodes);
    d4est_output_error(p4est, d4est_ops, d4est_geom, d4est_quad, level, error, local_eta2, local_nodes);
