...
)
{
  ...
  if (d4est_util_match_couple(section,"problem",name,"solve_after_level")) {
    D4EST_ASSERT(pconfig->solve_after_level == -1);
    pconfig->solve_after_level = atoi(value);
  }
...
)
{
  ...
  D4EST_CHECK_INPUT("problem", input.solve_after_level, -1);
...
)
{
  ...
  if (elem_data->tree == 12){
    ...
    printf("eta2_percentile, params->percentile, eta2 = %.25f, %d, %.25f, refine %d\n",eta2_percentile, params->percentile, eta2, ((eta2 >= eta2_percentile) || fabs(eta2 - eta2_percentile) < eta2*1e-4));
...
)
{
  ...
  for (int level = 0; level < d4est_amr->num_of_amr_steps + 1; ++level){
   ...
   int min_level, max_level;
    ...
    if (level > init_params.solve_after_level){
