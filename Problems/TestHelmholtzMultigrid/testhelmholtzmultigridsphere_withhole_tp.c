...
)
{
  ...
  else if (d4est_util_match_couple(section,"problem",name,"ip_flux_penalty")) {
...
)
{
   ...
   int vtk_nodes = 0;
    ...
    if (with_eta)
      sprintf(sol_save_as, "%s_amr_level_%d_sols_witheta", "helmholtz", level);
    else
      sprintf(sol_save_as, "%s_amr_level_%d_sols_noeta", "helmholtz", level);
