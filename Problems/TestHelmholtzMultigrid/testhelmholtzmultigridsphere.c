...
)
{
   ...
   int vtk_nodes = 0;
    ...
    if (with_eta)
      sprintf(sol_save_as, "%s_amr_level_%d_sols_witheta", "puncture", level);
    else
      sprintf(sol_save_as, "%s_amr_level_%d_sols_noeta", "puncture", level);
...
#if (P4EST_DIM)==3
  return 6. + helmholtz_fcn(x,y,z,u,NULL)*u;
#else
  return 4 + helmholtz_fcn(x,y,z,u,NULL)*u;
#endif
...
)
{
  ...
  d4est_mesh_init_field
   ...
   prob_vecs->flux_fcn_data = curved_gauss_primal_sipg_flux_dirichlet_fetch_fcns(boundary_fcn,ip_flux_params);
...
)
{
  ...
  d4est_element_data_init_new(p4est,
    ...
    amr_scheme_t* scheme = amr_curved_uniform_init();
  ...
  amr_curved_uniform_destroy(scheme);
