...
)
{
  ...
  for (int level = 0; level < num_of_levels-1){
...
)
{
  ...
  int compute_geometric_factors = (geometric_factors[level] == NULL);
  if (compute_geometric_factors) {
    updater->geometric_factors[level] = geometric_factors_init(p4est);
  }
    
  
  if (mg_data->mg_state == DOWNV_POST_COARSEN){
    curved_element_data_init_new(p4est,
                                 updater->geometric_factors[level],
                                 mg_data->dgmath_jit_dbase,
                                 updater->d4est_geom,
                                 updater->element_data_init_fcn,
                                 updater->user,
