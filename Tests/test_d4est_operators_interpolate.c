...
{  
  ...
  if (proc_rank == 0)
    printf("[D4EST_INFO]: options file = %s\n", (argc == 2) ? argv[1] :      "test_d4est_operators_interpolate.input");
  ...
  d4est_mesh_initial_extents_t* initial_grid_input = d4est_mesh_initial_extents_parse((argc == 2) ? argv[1] :      "test_d4est_operators_interpolate.input", d4est_geom);
  ...
  d4est_quadrature_t* d4est_quad = d4est_quadrature_new(p4est, d4est_ops, d4est_geom, (argc == 2) ? argv[1] :      "test_d4est_operators_interpolate.input", "quadrature", "[QUADRATURE]");
  ...
  d4est_vtk_save
    (
     p4est,
     d4est_ops,
     "test_d4est_operators_interpolate.input",
     "d4est_vtk",
     "[D4EST_VTK]",
  ...
  d4est_mesh_interpolate_data_t data = d4est_mesh_interpolate_at_tree_coord(p4est, d4est_ops, d4est_geom, (double []){.75,.75,.75}, 6, poly, 1);

  double ans = poly_fcn(data.xyz[0],data.xyz[1],data.xyz[2],NULL);
  printf("ans = %.15f\n", ans);
  printf("a,b,c, r,s,t, x,y,z, f_at_xyz, nodal_stride, id, err= %f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%d, %d, %d\n", data.abc[0], data.abc[1], data.abc[2], data.rst[0], data.rst[1], data.rst[2], data.xyz[0],data.xyz[1],data.xyz[2], data.f_at_xyz, data.nodal_stride, data.id, data.err);
