...
#define HACKED_P4EST_VTK_CELL_TYPE     11      /* VTK_VOXEL */
#else

/* #include <p4est_nodes.h> */
#define HACKED_P4EST_VTK_CELL_TYPE      8      /* VTK_PIXEL */
#endif /* !P4_TO_P8 */

#include <sc_io.h>
#include <element_data.h>

static const double hacked_p4est_vtk_scale = 0.95;
static const int    hacked_p4est_vtk_write_tree = 1;
static const int    hacked_p4est_vtk_write_level = 1;
static const int    hacked_p4est_vtk_write_rank = 1;
static const int    hacked_p4est_vtk_wrap_rank = 0;

#ifndef HACKED_P4EST_VTK_DOUBLES
#define HACKED_P4EST_VTK_FLOAT_NAME "Float32"
#define HACKED_P4EST_VTK_FLOAT_TYPE float
#else
#define HACKED_P4EST_VTK_FLOAT_NAME "Float64"
#define HACKED_P4EST_VTK_FLOAT_TYPE double
#endif

#ifndef HACKED_P4EST_VTK_BINARY
#define HACKED_P4EST_VTK_ASCII 1
#define HACKED_P4EST_VTK_FORMAT_STRING "ascii"
#else
#define HACKED_P4EST_VTK_FORMAT_STRING "binary"

static int
hacked_p4est_vtk_write_binary (FILE * vtkfile, char *numeric_data,
                        size_t byte_length)
{
#ifndef HACKED_P4EST_VTK_COMPRESSION
...
#endif /* HACKED_P4EST_VTK_COMPRESSION */
}

#endif /* HACKED_P4EST_VTK_BINARY */

void
hacked_p4est_vtk_write_file (p4est_t * p4est, p4est_geometry_t * geom,
                      const char *filename)
{
  hacked_p4est_vtk_write_all (p4est, geom,
                       hacked_p4est_vtk_scale,
                       hacked_p4est_vtk_write_tree, hacked_p4est_vtk_write_level,
                       hacked_p4est_vtk_write_rank, hacked_p4est_vtk_wrap_rank,
                       0, 0, filename);
}
                     ...
                     const char *filename, ...)
{
  ...
  retval = hacked_p4est_vtk_write_header (p4est, geom, scale,
  ...
  all = 0;
  for (i = 0; i < num_scalars; ++all, ++i) {
    retval = hacked_p4est_vtk_write_point_scalar (p4est, geom, filename,
                                           names[all], values[all]);
    SC_CHECK_ABORT (!retval,
                    P4EST_STRING "_vtk: Error writing point scalars");
  }
  for (i = 0; i < num_vectors; ++all, ++i) {
    retval = hacked_p4est_vtk_write_point_vector (p4est, geom, filename,
                                           names[all], values[all]);
    SC_CHECK_ABORT (!retval,
                    P4EST_STRING "_vtk: Error writing point vectors");
  }

  retval = hacked_p4est_vtk_write_footer (p4est, filename);
...
int
hacked_p4est_vtk_write_header (p4est_t * p4est, p4est_geometry_t * geom,
                        double scale,
                        int write_tree, int write_level,
                        int write_rank, int wrap_rank,
                        ...
                        const char *filename)
{
  ...
  p4est_locidx_t      il;
  HACKED_P4EST_VTK_FLOAT_TYPE *float_data;
...
#if defined HACKED_P4EST_VTK_BINARY && defined HACKED_P4EST_VTK_COMPRESSION
  ...
  float_data = P4EST_ALLOC (HACKED_P4EST_VTK_FLOAT_TYPE, 3 * Ntotal);

  /* write point position data */
  fprintf (vtufile, "        <DataArray type=\"%s\" Name=\"Position\""
           " NumberOfComponents=\"3\" format=\"%s\">\n",
           HACKED_P4EST_VTK_FLOAT_NAME, HACKED_P4EST_VTK_FORMAT_STRING);
  ...
  if (nodes == NULL) {
              ...
              if (geom != NULL) {
                ...
                for (j = 0; j < 3; ++j) {
                  float_data[3 * ((P4EST_CHILDREN) * quad_count + k) + j] =
                    (HACKED_P4EST_VTK_FLOAT_TYPE) XYZ[j];
                }
              }
              else {
                for (j = 0; j < 3; ++j) {
    ...
    for (zz = 0; zz < indeps->elem_count; ++zz) {
      ...
      if (geom != NULL) {
        ...
        geom->X (geom, jt, xyz, XYZ);
        for (j = 0; j < 3; ++j) {
          float_data[3 * zz + j] = (HACKED_P4EST_VTK_FLOAT_TYPE) XYZ[j];
        }
      }
      else {
        for (j = 0; j < 3; ++j) {
...
#ifdef HACKED_P4EST_VTK_DOUBLES
   ...
   * at a time.
   */
  retval = hacked_p4est_vtk_write_binary (vtufile, (char *) float_data,
  ...
  fprintf (vtufile, "        <DataArray type=\"%s\" Name=\"connectivity\""
           " format=\"%s\">\n", P4EST_VTK_LOCIDX, HACKED_P4EST_VTK_FORMAT_STRING);
#ifdef HACKED_P4EST_VTK_ASCII
  ...
  if (nodes == NULL) {
    ...
    retval =
      hacked_p4est_vtk_write_binary (vtufile, (char *) locidx_data,
                              sizeof (*locidx_data) * Ncorners);
  }
  else {
    retval =
      hacked_p4est_vtk_write_binary (vtufile, (char *) nodes->local_nodes,
                              sizeof (*locidx_data) * Ncorners);
  }
  fprintf (vtufile, "\n");
  if (retval) {
  ...
  fprintf (vtufile, "          ");
  retval = hacked_p4est_vtk_write_binary (vtufile, (char *) locidx_data,
  ...
  fprintf (vtufile, "        <DataArray type=\"UInt8\" Name=\"types\""
           " format=\"%s\">\n", HACKED_P4EST_VTK_FORMAT_STRING);
#ifdef HACKED_P4EST_VTK_ASCII
  fprintf (vtufile, "         ");
  for (il = 0, sk = 1; il < Ncells; ++il, ++sk) {
    fprintf (vtufile, " %d", HACKED_P4EST_VTK_CELL_TYPE);
  ...
  for (il = 0; il < Ncells; ++il)
    uint8_data[il] = HACKED_P4EST_VTK_CELL_TYPE;

  fprintf (vtufile, "          ");
  retval = hacked_p4est_vtk_write_binary (vtufile, (char *) uint8_data,
  ...
  if (write_tree || write_level || write_rank) {
    ...
    if (write_tree)
      printed +=
        snprintf (vtkCellDataString + printed, BUFSIZ - printed, "eta2");
    if (write_level)
      printed +=
  ...
  if (write_tree) {
    fprintf (vtufile, "        <DataArray type=\"%s\" Name=\"eta2\""
             " format=\"%s\">\n", P4EST_VTK_LOCIDX, HACKED_P4EST_VTK_FORMAT_STRING);
#ifdef HACKED_P4EST_VTK_ASCII
    fprintf (vtufile, "         ");
  ...
  if (write_level) {
    fprintf (vtufile, "        <DataArray type=\"%s\" Name=\"degree\""
             " format=\"%s\">\n", "UInt8", HACKED_P4EST_VTK_FORMAT_STRING);
#ifdef HACKED_P4EST_VTK_ASCII
    fprintf (vtufile, "         ");
    for (il = 0, sk = 1, jt = first_local_tree; jt <= last_local_tree; ++jt) {
      tree = p4est_tree_array_index (trees, jt);
      ...
      for (zz = 0; zz < num_quads; ++zz, ++sk, ++il) {
        ...
        fprintf (vtufile, " %d", (int) (((element_data_t*)(quad->p.user_data))->deg));
        if (!(sk % 20) && il != (Ncells - 1))
          fprintf (vtufile, "\n         ");
      }
    }
    ...
    for (il = 0, jt = first_local_tree; jt <= last_local_tree; ++jt) {
      ...
      for (zz = 0; zz < num_quads; ++zz, ++il) {
        quad = p4est_quadrant_array_index (quadrants, zz);
        uint8_data[il] = (uint8_t) (((element_data_t*)(quad->p.user_data))->deg);
      }
    }
  ...
  if (mpirank == 0) {
    ...
    if (write_level) {
      fprintf (pvtufile, "      "
               "<PDataArray type=\"%s\" Name=\"degree\" format=\"%s\"/>\n",
               "UInt8", HACKED_P4EST_VTK_FORMAT_STRING);
    }
    if (write_rank) {
      fprintf (pvtufile, "      "
...
int
hacked_p4est_vtk_write_point_scalar (p4est_t * p4est, p4est_geometry_t * geom,
                              const char *filename,
                              const char *scalar_name, const double *values)
{
  ...
  fprintf (vtufile, "        <DataArray type=\"%s\" Name=\"%s\""
           " format=\"%s\">\n",
           HACKED_P4EST_VTK_FLOAT_NAME, scalar_name, HACKED_P4EST_VTK_FORMAT_STRING);
...
#else
  float_data = P4EST_ALLOC (HACKED_P4EST_VTK_FLOAT_TYPE, Ncorners);
  for (il = 0; il < Ncorners; ++il) {
    float_data[il] = (HACKED_P4EST_VTK_FLOAT_TYPE) values[il];
  }
...
int
hacked_p4est_vtk_write_point_vector (p4est_t * p4est, p4est_geometry_t * geom,
                              const char *filename,
...
int
hacked_p4est_vtk_write_footer (p4est_t * p4est, const char *filename)
{
  char                vtufilename[BUFSIZ];
  int                 p;
  ...
  if (ferror (vtufile)) {
    P4EST_LERROR ("hacked_p4est_vtk: Error writing footer\n");
    fclose (vtufile);
    return -1;
  }
  if (fclose (vtufile)) {
    P4EST_LERROR ("hacked_p4est_vtk: Error closing footer\n");
    return -1;
  }
  vtufile = NULL;
  ...
  if (procRank == 0) {
    ...
    if (ferror (pvtufile)) {
      P4EST_LERROR ("hacked_p4est_vtk: Error writing parallel footer\n");
      fclose (visitfile);
      fclose (pvtufile);
      return -1;
    }
    if (fclose (pvtufile)) {
      fclose (visitfile);
      P4EST_LERROR ("hacked_p4est_vtk: Error closing parallel footer\n");
      return -1;
    }
