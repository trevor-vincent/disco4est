...
static void
d4est_solver_multigrid_matrix_operator_restriction_callback
(
...
static void
d4est_solver_multigrid_matrix_operator_update_callback
(
 p4est_t* p4est,
 int level,
...
)
{
  ...
  if(mg_data->mg_state == PRE_V){
    if (matrix_op->completed_alloc == 0){
      ...
      matrix_op->total_matrix_nodes_on_d4est_solver_multigrid = matrix_op->matrix_nodes_on_level[level];
    }
    ...
    if (matrix_op->completed_alloc == 0){
      ...
      matrix_op->total_matrix_nodes_on_d4est_solver_multigrid += matrix_op->matrix_nodes_on_level[level-1];
      matrix_op->matrix_at0 = P4EST_REALLOC
                              (
                               matrix_op->matrix_at0,
                               double,
                               matrix_op->total_matrix_nodes_on_d4est_solver_multigrid
                              );
    }
...
)
{
  ...
  matrix_op->total_matrix_nodes_on_d4est_solver_multigrid = -1;
  ...
  user_callbacks->mg_restrict_user_callback = d4est_solver_multigrid_matrix_operator_restriction_callback;
  user_callbacks->update = d4est_solver_multigrid_matrix_operator_update_callback;
...
(
 ...
 void* fofv_ctx,
 d4est_solver_multigrid_matrix_op_t* matrix_op
)
{
  int nodal_stride = 0;
...
void
d4est_solver_multigrid_matrix_operator_destroy(d4est_solver_multigrid_user_callbacks_t* user_callbacks)
{
  d4est_solver_multigrid_matrix_op_t* matrix_op = user_callbacks->user;
