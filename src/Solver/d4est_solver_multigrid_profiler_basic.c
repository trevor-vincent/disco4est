...
void
d4est_solver_multigrid_profiler_basic_destroy
(
...
static
void
d4est_solver_multigrid_profiler_basic_update
(
 p4est_t* p4est,
 int level,
...
)
{
  ...
  if(mg_data->mg_state == START){
    ...
    d4est_solver_multigrid_profiler_basic_print(profiler, p4est->mpirank);
  }  
  else {
    return;
  }
...
d4est_solver_multigrid_profiler_basic_init
(
)
{
  d4est_solver_multigrid_profiler_t* profiler = P4EST_ALLOC(d4est_solver_multigrid_profiler_t, 1);
  d4est_solver_multigrid_profiler_basic_data_t* profiler_data = P4EST_ALLOC(d4est_solver_multigrid_profiler_basic_data_t, 1);
  profiler->update = d4est_solver_multigrid_profiler_basic_update;
