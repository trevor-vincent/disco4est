...
{
  d4est_solver_multigrid_mesh_analyzer_t* mesh_analyzer = mesh_analyzer->user;
  free(mesh_analyzer->input_file_);
...
)
{
  ...
  if(mg_data->mg_state == START){
    ...
    for (int lev = mesh_analyzer->levels; lev >= 0; lev--){
...
)
{
  ...
  strcpy(mesh_analyzer->input_file, name);
