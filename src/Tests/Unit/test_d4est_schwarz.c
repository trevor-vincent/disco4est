...
static void 
test_d4est_iterate_refine_only_one_element
(
...
int main(int argc, char *argv[])
{
  ...
  if (proc_rank == 0)
    printf("[D4EST_INFO]: options file = %s\n", (argc == 2) ? argv[1] :      "test_d4est_schwarz_metadata.input");
  ...
  d4est_mesh_initial_extents_t* initial_grid_input = d4est_mesh_initial_extents_parse((argc == 2) ? argv[1] :      "test_d4est_schwarz_metadata.input", d4est_geom);
  ...
  d4est_quadrature_t* d4est_quad = d4est_quadrature_new(p4est, d4est_ops, d4est_geom, (argc == 2) ? argv[1] :      "test_d4est_schwarz_metadata.input", "quadrature");
  ...
  if (add_amr_in_one_element){
    ...
    scheme->mark_elements = test_d4est_iterate_refine_only_one_element;
  ...
  const char** vtk_int_arrays = P4EST_ALLOC(char*, p4est->local_num_quadrants + 2);
  int** subdomains = P4EST_ALLOC(int*, p4est->local_num_quadrants + 2);  
  int* element_id = P4EST_ALLOC(int, p4est->local_num_quadrants);

  for (int i = 0; i < p4est->local_num_quadrants; i++){
    d4est_element_data_t* ed = d4est_factors->element_data[i];
    element_id[i] = ed->id;
    subdomains[i] = P4EST_ALLOC(int, p4est->local_num_quadrants);
  }

  int num_nodes_overlap = 2;
  
  d4est_solver_schwarz_data_t* schwarz_data = d4est_solver_schwarz_init(p4est, num_nodes_overlap);
  
  d4est_vtk_save
    (
     p4est,
     d4est_ops,
     "test_d4est_schwarz_metadata.input",
     "d4est_vtk",
     NULL,
     NULL,
     NULL,
