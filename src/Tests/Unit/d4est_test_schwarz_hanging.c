...
int main(int argc, char *argv[])
{
  ...
  sprintf(input_file, "%s", (argc == 2) ? argv[1] : "d4est_test_schwarz_hanging.input");
  ...
  d4est_test_schwarz_cubed_sphere_hanging_t test_hanging_input = d4est_test_schwarz_cubed_sphere_hanging_input(input_file);
  ...
  printf("Elements in mesh = %d\n", p4est->local_num_quadrants);
  printf("Nodes in mesh = %d\n", local_sizes.local_nodes);
...
d4est_solver_schwarz_t* schwarz =
 ...
 for (int i = 0; i < schwarz_metadata->num_subdomains; i++){
   ...
   /* for (int el = 0; el < sub_data->num_elements; el++){ */
     /* printf("subdomain element %d, mesh id %d, nodal stride %d\n",el ,sub_data->element_metadata[el].id,sub_data->element_metadata[el].restricted_nodal_stride); */
