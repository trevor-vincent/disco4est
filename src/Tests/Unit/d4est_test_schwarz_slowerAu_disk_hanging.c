...
} d4est_test_schwarz_disk_hanging_t;



static
int d4est_test_schwarz_disk_hanging_input_handler
(
 void* user,
 const char* section,
 const char* name,
...
{
  d4est_test_schwarz_disk_hanging_t* pconfig = (d4est_test_schwarz_disk_hanging_t*)user;
...
static
d4est_test_schwarz_disk_hanging_t
d4est_test_schwarz_disk_hanging_input
(
 const char* input_file
)
{
  d4est_test_schwarz_disk_hanging_t input;

  input.schwarz_iter = -2;
  input.element_to_hrefine = -2;
  
  if (ini_parse(input_file, d4est_test_schwarz_disk_hanging_input_handler, &input) < 0) {
    printf("input_file = %s\n", input_file);
...
int main(int argc, char *argv[])
{
...
#endif
  const char* default_input_file = "d4est_test_schwarz_disk_hanging.input";
  ...
  d4est_test_schwarz_disk_hanging_t test_hanging_input = d4est_test_schwarz_disk_hanging_input(default_input_file);
 ...
 /*   ); */
  ...
  for (int i = 0; i < schwarz_data->num_subdomains; i++){
   ...
   u_sub_sum += d4est_util_sum_array_dbl(u_sub, sub_data->restricted_nodal_size);
   Au_sub_sum += d4est_util_sum_array_dbl(Au_sub, sub_data->restricted_nodal_size);

   /* DEBUG_PRINT_ARR_DBL(Au_sub, sub_data->restricted_nodal_size); */
   ...
   printf("u sub total sum = %.15f\n", u_sub_sum);
   printf("Au sub total sum = %.15f\n", Au_sub_sum);
