...
int main(int argc, char *argv[])
{
  ...
  if (proc_rank == 0)
    printf("[D4EST_INFO]: options file = %s\n", (argc == 2) ? argv[1] :      "d4est_test_schwarz_singlecore_Au_3d.input");
  ...
  d4est_mesh_initial_extents_t* initial_grid_input = d4est_mesh_initial_extents_parse((argc == 2) ? argv[1] :      "d4est_test_schwarz_singlecore_Au_3d.input", d4est_geom);
  ...
  d4est_quadrature_t* d4est_quad = d4est_quadrature_new(p4est, d4est_ops, d4est_geom, (argc == 2) ? argv[1] :      "d4est_test_schwarz_singlecore_Au_3d.input", "quadrature");
  ...
  char** dbl_subdomain_names = P4EST_ALLOC(char*, 2*p4est->local_num_quadrants + 4);
  double** dbl_subdomains = P4EST_ALLOC(double*, 2*p4est->local_num_quadrants + 4);
  ...
  for (int i = 0; i < schwarz_data->num_subdomains; i++){
    ...
    d4est_solver_schwarz_convert_restricted_subdomain_field_to_global_nodal_field
      (
       ...
       dbl_subdomains[p4est->local_num_quadrants + i],
       p4est->mpirank,
       i,
       local_sizes.local_nodes,
       FIELD_NOT_ZEROED
    ...
    /* double* tmp = &Apoly_vec_over_subdomains[sub_data->restricted_nodal_stride]; */
    /* DEBUG_PRINT_ARR_DBL(tmp, sub_data->restricted_nodal_size); */
    
    sprintf(dbl_subdomain_names[i], "poly_vec_%d", i);
    sprintf(dbl_subdomain_names[i + p4est->local_num_quadrants], "Apoly_vec_%d", i);
  
  }

  dbl_subdomains[2*p4est->local_num_quadrants] = poly_vec;
  dbl_subdomains[2*p4est->local_num_quadrants + 1] = Apoly_vec;
  dbl_subdomains[2*p4est->local_num_quadrants + 2] = Apoly_vec_compare;

  dbl_subdomain_names[2*p4est->local_num_quadrants] = P4EST_ALLOC(char, 25);
  dbl_subdomain_names[2*p4est->local_num_quadrants + 1] = P4EST_ALLOC(char, 25);
  dbl_subdomain_names[2*p4est->local_num_quadrants + 2] = P4EST_ALLOC(char, 25);

  sprintf(dbl_subdomain_names[2*p4est->local_num_quadrants], "poly_vec");
  sprintf(dbl_subdomain_names[2*p4est->local_num_quadrants + 1], "Apoly_vec");
  sprintf(dbl_subdomain_names[2*p4est->local_num_quadrants + 2], "Apoly_vec_compare");
  
  dbl_subdomain_names[2*p4est->local_num_quadrants + 3] = NULL;
  dbl_subdomains[2*p4est->local_num_quadrants + 3] = NULL;

    d4est_vtk_save
    (
     p4est,
     d4est_ops,
     ((P4EST_DIM) == 3) ? "d4est_test_schwarz_singlecore_Au_3d.input" : "d4est_test_schwarz_singlecore_Au_2d.input",
     "d4est_vtk",
  ...
  for (int i = 0; i < 2*p4est->local_num_quadrants; i++){
    /* P4EST_FREE(dbl_subdomain_names[i]); */
    P4EST_FREE(dbl_subdomains[i]);
  }
  
  for (int i = 0; i < 2*p4est->local_num_quadrants + 4; i++){
    P4EST_FREE(dbl_subdomain_names[i]);
    /* P4EST_FREE(dbl_subdomains[i]); */
  }
