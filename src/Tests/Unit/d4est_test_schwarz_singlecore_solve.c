...
int main(int argc, char *argv[])
{
  ...
  if (add_amr_in_one_element){
    ...
    double* u_analytic= P4EST_ALLOC(double, local_sizes.local_nodes);
    double* f = P4EST_ALLOC(double, local_sizes.local_nodes);
    double* rhs = P4EST_ALLOC(double, local_sizes.local_nodes);
    double* r = P4EST_ALLOC_ZERO(double, local_sizes.local_nodes);
    double* u = P4EST_ALLOC_ZERO(double, local_sizes.local_nodes);
    double* error = P4EST_ALLOC_ZERO(double, local_sizes.local_nodes);
  ...
  for (int i = 0; i < num_corrections; i++){
    ...
    d4est_util_compute_error_array(u, u_analytic, error, local_sizes.local_nodes);
