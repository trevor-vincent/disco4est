...
} testd4est_constant_density_star_fcns_t;
...
/* ) */
/* { */
/*   testd4est_constant_density_star_fcns_t* data = user_ctx; */
...
static double
testd4est_constant_density_star_initial_guess
(
 double x,
 double y,
 double z,
...
int main(int argc, char *argv[])
{
  ...
  const char* input_file = "testd4est_constant_density_star_fcns.input";
  /*  */
  ...
  testd4est_constant_density_star_fcns_t deg_data;
  D4EST_ASSERT(argc == 4);
  ...
  for (int level = 0; level < d4est_amr->num_of_amr_steps; ++level){
    ...
    for (int i = 0; i < NUM_OF_TRIALS; i++){
      ...
      int local_nodes = d4est_mesh_update
        ...
        elliptic_eqns.apply_lhs = constant_density_star_apply_jac;
        elliptic_eqns.build_residual = constant_density_star_build_residual;
        elliptic_eqns.user = &constant_density_star_params;

        double* poly_vec = P4EST_ALLOC(double, local_nodes);

        d4est_mesh_init_field(p4est, poly_vec, constant_density_star_analytic_solution_fcn, d4est_ops, d4est_geom, &constant_density_star_params);
        ...
        printf("elem %d deg %d deg_v %d deg_m %d jacl %.15f resl %.15f jerr %.15f rerr %.15f resl2 %.15f\n", p4est->local_num_quadrants, deg_data.deg, deg_data.deg_volume_quad, deg_data.deg_mortar_quad, jac_last_node[i], res_last_node[i], jac_last_node_error[i],  res_last_node_error[i], l2_norm);
