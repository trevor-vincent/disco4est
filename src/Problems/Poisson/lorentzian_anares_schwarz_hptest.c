...
)
{
  ...
  if (elem_data->tree == *tree_to_refine && elem_data->tree_quadid == 0){
...
)
{
  ...
  else if (d4est_util_match_couple(section,"problem",name,"tree_to_refine")) {
    D4EST_ASSERT(pconfig->tree_to_refine == -1);
...
)
{
  ...
  D4EST_CHECK_INPUT("problem", input.tree_to_refine, -1);
...
)
{
  ...
  poisson_lorentzian_init_params_t init_params = poisson_lorentzian_init_params_input
    ...
    d4est_amr_temp->mpirank = p4est->mpirank;
    d4est_amr_temp->scheme = scheme;
    d4est_amr_temp->balance_log = NULL;
  ...
  d4est_mesh_local_sizes_t local_sizes = d4est_mesh_update
    ...
    initial_extents->initial_nodes = local_sizes.local_nodes;
  /* } */
