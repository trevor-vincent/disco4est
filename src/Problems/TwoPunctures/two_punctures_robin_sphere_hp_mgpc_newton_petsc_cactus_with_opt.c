...
)
{
  ...
  d4est_laplacian_with_opt_flux_data_t* flux_data_for_bi = d4est_laplacian_with_opt_flux_new(p4est, input_file, BC_DIRICHLET, &bc_data_for_bi);
  
  d4est_laplacian_with_opt_flux_data_t* flux_data_for_jac = d4est_laplacian_with_opt_flux_new(p4est, input_file, BC_ROBIN, &bc_data_for_jac);
  
  d4est_laplacian_with_opt_flux_data_t* flux_data_for_res = d4est_laplacian_with_opt_flux_new(p4est, input_file,  BC_ROBIN, &bc_data_for_res);
  ...
  for (int level = initial_level; level < d4est_amr->num_of_amr_steps + 1; ++level){
   ...
   int min_level, max_level;
    ...
    d4est_krylov_pc_t* pc = d4est_krylov_pc_multigrid_create(mg_data, two_puncturesd4est_krylov_pc_setup_fcn);
