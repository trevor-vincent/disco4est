...
static void
d4est_amr_puncture_finder_refine_callback
(
  ...
  d4est_mesh_data_t* d4est_factors = params->d4est_factors;
  ...
  xyz[0] = &d4est_factors->xyz[ed->nodal_stride];
  xyz[1] = &d4est_factors->xyz[local_nodes + ed->nodal_stride];
  xyz[2] = &d4est_factors->xyz[2*local_nodes + ed->nodal_stride];

  d4est_geometry_compute_bounds(xyz, ed->deg, xi, xf);
...
)
{
  ...
  else if (d4est_util_match_couple(section,"problem",name,"level_puncture_finder_stop")) {
    D4EST_ASSERT(pconfig->level_puncture_finder_stop == -1);
    pconfig->level_puncture_finder_stop = atoi(value);
  }
...
{ 
  ...
  d4est_amr_t* d4est_amr_smooth_pred =
    d4est_amr_init
    (
     p4est,
     input_file,
  ...
  puncture_finder_scheme->mark_elements = d4est_amr_puncture_finder_refine_callback;
  puncture_finder_scheme->amr_scheme_data = &two_punctures_params;
  puncture_finder_scheme->destroy = NULL;
  d4est_amr_puncture_finder->mpirank = p4est->mpirank;
  d4est_amr_puncture_finder->num_of_amr_steps = d4est_amr_smooth_pred->num_of_amr_steps;
  d4est_amr_puncture_finder->scheme = puncture_finder_scheme;
  d4est_amr_puncture_finder->balance_log = NULL;
  d4est_amr_puncture_finder->refinement_log = NULL;
  d4est_amr_puncture_finder->initial_log = NULL;
  d4est_amr_puncture_finder->max_degree = 1000;
  ...
  for (int level = initial_level; level < d4est_amr_smooth_pred->num_of_amr_steps + 1; ++level){

    d4est_amr_t* d4est_amr = (level >= level_puncture_finder_stop) ? d4est_amr_smooth_pred : d4est_amr_puncture_finder;
  ...
  if (!init_params.do_not_solve){
    ...
    if (level != d4est_amr->num_of_amr_steps && level != 0){
      if (d4est_amr == d4est_amr_smooth_pred){
  ...
  d4est_amr_destroy(d4est_amr_smooth_pred);
  d4est_amr_destroy(d4est_amr_puncture_finder);
  P4EST_FREE(puncture_finder_scheme);
