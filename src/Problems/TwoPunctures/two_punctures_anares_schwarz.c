...
{ 
  ...
  for (int level = initial_level; level < d4est_amr->num_of_amr_steps + 1; ++level){
    ...
    two_punctures_cactus_schwarz_t cactus_schwarz;
    cactus_schwarz.problem_ctx = &ctx; //problem_ctx;
    cactus_schwarz.u0_ghost_data = d4est_ghost_data_init
                                    (
                                     p4est,
                                     *d4est_ghost,
                                     &field_type,
    ...
    d4est_krylov_pc_t* pc = d4est_krylov_pc_schwarz_create(input_file, schwarz, NULL);
    ...
    if (level != d4est_amr->num_of_amr_steps && level != 0){
      d4est_checkpoint_save
        (
         ...
         d4est_amr,
         d4est_factors,
         (const char * []){"u", "predictor", NULL},
         (hid_t []){H5T_NATIVE_DOUBLE, H5T_NATIVE_DOUBLE},
         (int []){prob_vecs.local_nodes, p4est->local_num_quadrants},
         (void* []){prob_vecs.u, smooth_pred_data->predictor}
        );
    }
    ...
    d4est_krylov_pc_schwarz_destroy(pc);
    
    P4EST_FREE(error_l2);
    P4EST_FREE(estimator);


    d4est_ghost_data_destroy(cactus_schwarz.u0_ghost_data);
    d4est_laplacian_flux_destroy(cactus_schwarz.flux_data_for_jac_nonopt);
