...
#define P4EST_VTK_EXT_CELL_TYPE     11      /* VTK_VOXEL */
#else

/* #include <p4est_nodes.h> */
#define P4EST_VTK_EXT_CELL_TYPE      8      /* VTK_PIXEL */
#endif /* !P4_TO_P8 */

#include <sc_io.h>
#include <d4est_element_data.h>

static const double p4est_vtk_ext_scale = 0.95;
static const int    p4est_vtk_ext_write_tree = 1;
static const int    p4est_vtk_ext_write_level = 1;
static const int    p4est_vtk_ext_write_rank = 1;
static const int    p4est_vtk_ext_wrap_rank = 0;

#ifndef P4EST_VTK_EXT_DOUBLES
#define P4EST_VTK_EXT_FLOAT_NAME "Float32"
#define P4EST_VTK_EXT_FLOAT_TYPE float
#else
#define P4EST_VTK_EXT_FLOAT_NAME "Float64"
#define P4EST_VTK_EXT_FLOAT_TYPE double
#endif

#ifndef P4EST_VTK_EXT_BINARY
#define P4EST_VTK_EXT_ASCII 1
#define P4EST_VTK_EXT_FORMAT_STRING "ascii"
#else
#define P4EST_VTK_EXT_FORMAT_STRING "binary"

static int
p4est_vtk_ext_write_binary (FILE * vtkfile, char *numeric_data,
                        size_t byte_length)
{
#ifndef P4EST_VTK_EXT_COMPRESSION
...
#endif /* P4EST_VTK_EXT_COMPRESSION */
}

#endif /* P4EST_VTK_EXT_BINARY */

void
p4est_vtk_ext_write_file (p4est_t * p4est, p4est_geometry_t * geom,
                      const char *filename)
{
  p4est_vtk_ext_write_all (p4est, geom,
                       p4est_vtk_ext_scale,
                       p4est_vtk_ext_write_tree, p4est_vtk_ext_write_level,
                       p4est_vtk_ext_write_rank, p4est_vtk_ext_wrap_rank,
                       0, 0, filename);
}
                     ...
                     const char *filename, ...)
{
  ...
  retval = p4est_vtk_ext_write_header (p4est, geom, scale,
  ...
  all = 0;
  for (i = 0; i < num_scalars; ++all, ++i) {
    retval = p4est_vtk_ext_write_point_scalar (p4est, geom, filename,
                                           names[all], values[all]);
    SC_CHECK_ABORT (!retval,
                    P4EST_STRING "_vtk: Error writing point scalars");
  }
  for (i = 0; i < num_vectors; ++all, ++i) {
    retval = p4est_vtk_ext_write_point_vector (p4est, geom, filename,
                                           names[all], values[all]);
    SC_CHECK_ABORT (!retval,
                    P4EST_STRING "_vtk: Error writing point vectors");
  }

  retval = p4est_vtk_ext_write_footer (p4est, filename);
...
int
p4est_vtk_ext_write_header (p4est_t * p4est, p4est_geometry_t * geom,
                        double scale,
                        int write_tree, int write_level,
                        int write_rank, int wrap_rank,
                        ...
                        const char *filename)
{
  ...
  p4est_locidx_t      il;
  P4EST_VTK_EXT_FLOAT_TYPE *float_data;
...
#if defined P4EST_VTK_EXT_BINARY && defined P4EST_VTK_EXT_COMPRESSION
  ...
  float_data = P4EST_ALLOC (P4EST_VTK_EXT_FLOAT_TYPE, 3 * Ntotal);

  /* write point position data */
  fprintf (vtufile, "        <DataArray type=\"%s\" Name=\"Position\""
           " NumberOfComponents=\"3\" format=\"%s\">\n",
           P4EST_VTK_EXT_FLOAT_NAME, P4EST_VTK_EXT_FORMAT_STRING);
  ...
  if (nodes == NULL) {
              ...
              if (geom != NULL) {
                ...
                for (j = 0; j < 3; ++j) {
                  float_data[3 * ((P4EST_CHILDREN) * quad_count + k) + j] =
                    (P4EST_VTK_EXT_FLOAT_TYPE) XYZ[j];
                }
              }
              else {
                for (j = 0; j < 3; ++j) {
    ...
    for (zz = 0; zz < indeps->elem_count; ++zz) {
      ...
      if (geom != NULL) {
        ...
        geom->X (geom, jt, xyz, XYZ);
        for (j = 0; j < 3; ++j) {
          float_data[3 * zz + j] = (P4EST_VTK_EXT_FLOAT_TYPE) XYZ[j];
        }
      }
      else {
        for (j = 0; j < 3; ++j) {
   ...
   * at a time.
   */
  retval = p4est_vtk_ext_write_binary (vtufile, (char *) float_data,
  ...
  fprintf (vtufile, "        <DataArray type=\"%s\" Name=\"connectivity\""
           " format=\"%s\">\n", P4EST_VTK_LOCIDX, P4EST_VTK_EXT_FORMAT_STRING);
#ifdef P4EST_VTK_EXT_ASCII
  ...
  if (nodes == NULL) {
    ...
    retval =
      p4est_vtk_ext_write_binary (vtufile, (char *) locidx_data,
                              sizeof (*locidx_data) * Ncorners);
  }
  else {
    retval =
      p4est_vtk_ext_write_binary (vtufile, (char *) nodes->local_nodes,
                              sizeof (*locidx_data) * Ncorners);
  }
  fprintf (vtufile, "\n");
  if (retval) {
  ...
  fprintf (vtufile, "          ");
  retval = p4est_vtk_ext_write_binary (vtufile, (char *) locidx_data,
  ...
  fprintf (vtufile, "        <DataArray type=\"UInt8\" Name=\"types\""
           " format=\"%s\">\n", P4EST_VTK_EXT_FORMAT_STRING);
#ifdef P4EST_VTK_EXT_ASCII
  fprintf (vtufile, "         ");
  for (il = 0, sk = 1; il < Ncells; ++il, ++sk) {
    fprintf (vtufile, " %d", P4EST_VTK_EXT_CELL_TYPE);
  ...
  for (il = 0; il < Ncells; ++il)
    uint8_data[il] = P4EST_VTK_EXT_CELL_TYPE;

  fprintf (vtufile, "          ");
  retval = p4est_vtk_ext_write_binary (vtufile, (char *) uint8_data,
  ...
  if (write_level) {
    fprintf (vtufile, "        <DataArray type=\"%s\" Name=\"degree\""
             " format=\"%s\">\n", "UInt8", P4EST_VTK_EXT_FORMAT_STRING);
#ifdef P4EST_VTK_EXT_ASCII
    fprintf (vtufile, "         ");
    for (il = 0, sk = 1, jt = first_local_tree; jt <= last_local_tree; ++jt) {
      tree = p4est_tree_array_index (trees, jt);
      ...
      for (zz = 0; zz < num_quads; ++zz, ++sk, ++il) {
        ...
        fprintf (vtufile, " %d", (int) (((d4est_element_data_t*)(quad->p.user_data))->deg));
        if (!(sk % 20) && il != (Ncells - 1))
          fprintf (vtufile, "\n         ");
      }
    }
    ...
    for (il = 0, jt = first_local_tree; jt <= last_local_tree; ++jt) {
      ...
      for (zz = 0; zz < num_quads; ++zz, ++il) {
        quad = p4est_quadrant_array_index (quadrants, zz);
        uint8_data[il] = (uint8_t) (((d4est_element_data_t*)(quad->p.user_data))->deg);
      }
    }
...
int
p4est_vtk_ext_write_point_scalar (p4est_t * p4est, p4est_geometry_t * geom,
                              const char *filename,
                              const char *scalar_name, const double *values)
{
  ...
  fprintf (vtufile, "        <DataArray type=\"%s\" Name=\"%s\""
           " format=\"%s\">\n",
           P4EST_VTK_EXT_FLOAT_NAME, scalar_name, P4EST_VTK_EXT_FORMAT_STRING);
...
#else
  float_data = P4EST_ALLOC (P4EST_VTK_EXT_FLOAT_TYPE, Ncorners);
  for (il = 0; il < Ncorners; ++il) {
    float_data[il] = (P4EST_VTK_EXT_FLOAT_TYPE) values[il];
  }
...
int
p4est_vtk_ext_write_point_vector (p4est_t * p4est, p4est_geometry_t * geom,
                              const char *filename,
...
int
p4est_vtk_ext_write_footer (p4est_t * p4est, const char *filename)
{
  char                vtufilename[BUFSIZ];
  int                 p;
  ...
  if (ferror (vtufile)) {
    P4EST_LERROR ("p4est_vtk_ext: Error writing footer\n");
    fclose (vtufile);
    return -1;
  }
  if (fclose (vtufile)) {
    P4EST_LERROR ("p4est_vtk_ext: Error closing footer\n");
    return -1;
  }
  vtufile = NULL;
  ...
  if (procRank == 0) {
    ...
    if (ferror (pvtufile)) {
      P4EST_LERROR ("p4est_vtk_ext: Error writing parallel footer\n");
      fclose (visitfile);
      fclose (pvtufile);
      return -1;
    }
    if (fclose (pvtufile)) {
      fclose (visitfile);
      P4EST_LERROR ("p4est_vtk_ext: Error closing parallel footer\n");
      return -1;
    }
